...
function setup() {
  ...
  const context = vm.createContext({ document, console, navigator: { userAgent: 'unit-test' }, window: { addEventListener() {} }, fetch: async (url, options = {}) => {
    ...
    return { text: async () => JSON.stringify(mode === 'reject' ? { status: 'error', retrySafe: true, message: '庫存不足' } : mode === 'duplicate' ? { status: 'duplicate' } : mode === 'no-id' ? { status: 'success' } : { status: 'success', orderId: 'TEST-' + requests.length }) };
  } });
...
(async () => {
  ...
  assert.equal(r.run('savedOrders[0].data.grandTotal'),305);
  assert.equal(r.run('savedOrders[0].data.selectedItems[0].price'),120);
  assert.match(r.run('currentOrderText'),/305元/);
  assert.match(r.run('currentOrderText'),/120元/);
  assert.match(r.run('currentOrderText'),/後台最新價格/);
  assert.equal(r.run('state.quantities.veg'),0);
  const u=setup();u.qty('veg',1);u.customer();
  u.run(`fetch = async () => ({text: async () => JSON.stringify({status:'uncertain',retrySafe:false,message:'write failed'})});`);
  await u.run('prepareSavedOrder()');assert.equal(u.run('unresolvedSubmission'),true);assert.equal(u.run('state.quantities.veg'),1);
  const malformed=setup();malformed.qty('veg',1);malformed.customer();
  malformed.run(`fetch = async () => ({text: async () => JSON.stringify({status:'success',subtotal:100,shipping:65,total:1})});`);
  await malformed.run('prepareSavedOrder()');assert.equal(malformed.run('unresolvedSubmission'),true);
  console.log('PASS server repricing appears in receipt/LINE, uncertain write protected, inconsistent totals not accepted');
